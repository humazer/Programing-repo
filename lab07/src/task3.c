...
int luckyticket_for(int num) {
	...
	if (sumfirstnum == sumlastnum) { // Проверяем везучий или нет через For
		resultFor = 1;
	}
	return resultFor;
}
...
int luckyticket_while(int num) {
	...
	if (sumfirstnumWhile == sumlastnumWhile) { // Проверяем везучий или нет через While
		resultWhile = 1;
	}
	return resultWhile;	
}
...
int luckyticket_dowhile(int num) {
	...
	if (sumfirstnumDoWhile == sumlastnumDoWhile) { // Проверяем везучий или нет через DoWhile 
		resultDoWhile = 1;
	}
